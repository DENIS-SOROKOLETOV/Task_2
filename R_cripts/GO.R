...
for (deg_file in deg_files) {
  ...
  has_plot <- make_go_plot(
    res_tbl = go_res,
    title_text = paste0(contrast_id, " | GO enrichment"),
    out_path = plot_path,
    top_n_per_ontology = 10
  )
  ...
  if (length(pair_set_files) > 0) {
    for (set_file in pair_set_files) {
      ...
      common_unique_summary_rows[[set_name]] <- analyze_go_for_gene_set(
        ...
        set_id = set_name,
        title_text = paste0("Venn set | ", set_name),
  ...
  for (uf in unique_files) {
    ...
    for (cid in names(split_sets)) {
      ...
      common_unique_summary_rows[[set_name]] <- analyze_go_for_gene_set(
        ...
        set_id = set_name,
        title_text = paste0("Venn unique set | ", cid),
        out_dir = common_unique_output_dir
      )
    }
  }
...
report_lines <- c(
  "GO функциональный анализ завершен.",
  "Метод: over-representation analysis (гипергеометрический тест) по GO-терминам из GAF.",
  "Коррекция множественного тестирования: BH (qvalue).",
  "Порог значимости термов: qvalue < 0.05.",
  "Визуализация: горизонтальные столбчатые диаграммы top-термов по -log10(qvalue) отдельно по онтологиям.",
  "Дополнительно выполнен GO-анализ для пересечений и уникальных наборов DEG из Venn_results; результаты сохранены в Enrichment_results/GO/CommonUnique_GO."
)
writeLines(report_lines, con = file.path(output_dir, "GO_report.txt"))
