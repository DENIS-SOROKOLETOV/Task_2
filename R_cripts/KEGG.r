...
deg_files <- list.files(de_dir, pattern = "_DEG\\.tsv$", full.names = TRUE)
...
save_top_plot <- function(tbl, output_path, title_text, top_n = 15) {
  ...
  tbl <- tbl[!is.na(tbl$padj), , drop = FALSE]
  if (nrow(tbl) == 0) {
    return(invisible(NULL))
  }
...
get_de_universe <- function(de_root) {
  all_files <- list.files(de_root, pattern = "_all_genes\\.tsv$", full.names = TRUE)
...
for (deg_file in deg_files) {
  ...
  all_file <- file.path(de_dir, paste0(contrast_id, "_all_genes.tsv"))
  ...
  save_top_plot(
    ...
    title_text = paste("KEGG enrichment:", contrast_id)
  )
...
write.table(summary_tbl, file.path(output_dir, "enrichment_summary.tsv"), sep = "\t", row.names = FALSE, quote = FALSE)
...
if (length(de_universe) > 0 && dir.exists(venn_dir)) {
  ...
  for (set_file in venn_set_files) {
    ...
    save_top_plot(
      ...
      title_text = paste("KEGG enrichment (Venn set):", set_id)
    )
    ...
    if (all(c("contrast_id", "Geneid") %in% colnames(core_unique_tbl))) {
      ...
      for (cid in contrast_ids) {
        ...
        save_top_plot(
          tbl = kegg_res,
          output_path = file.path(venn_output_dir, paste0(set_id, "_KEGG_top_terms.png")),
          title_text = paste("KEGG enrichment (Venn unique core):", cid)
        )
        venn_summary_rows[[set_id]] <- data.frame(
          set_id = set_id,
...
report_lines <- c(
  "Функциональный анализ DEG завершен.",
  "Использованы KEGG pathways (KEGG REST, организм fgr).",
  "Метод обогащения: over-representation analysis, точный критерий Фишера, поправка BH (FDR).",
  "Порог значимости: FDR < 0.05.",
  "Дополнительно выполнен enrichment для наборов из Venn_results (intersection/unique).",
  "Результаты сохранены в папке Enrichment_results/KEGG."
)
writeLines(report_lines, con = file.path(output_dir, "Enrichment_report.txt"))
